...
export function FloorPlanCanvas({
  ...
  const drawRooms = useCallback(
    (
      ...
      // Use sub-rects for composite rooms to correctly detect all adjacencies
      const adjacencyThreshold = 0.5; // bbox units
      const drawnLabels: { x: number; y: number }[] = []; // avoid duplicate labels

      for (let i = 0; i < bboxes.length; i++) {
        for (let j = i + 1; j < bboxes.length; j++) {
          // Get all individual rects for each room (sub-rects if composite, else bbox)
          const rectsI = effectiveSubRects[i] ?? [bboxes[i]];
          const rectsJ = effectiveSubRects[j] ?? [bboxes[j]];

          // Check every rect-to-rect pair across the two rooms
          for (const [aymin, axmin, aymax, axmax] of rectsI) {
            for (const [bymin, bxmin, bymax, bxmax] of rectsJ) {
              ...
              for (const wallX of vPairs) {
                ...
                // Skip if a label is already near this position
                if (drawnLabels.some(l => Math.abs(l.x - sx) < 10 && Math.abs(l.y - labelY) < 10)) continue;
              ...
              for (const wallY of hPairs) {
                ...
                // Skip if a label is already near this position
                if (drawnLabels.some(l => Math.abs(l.x - labelX) < 10 && Math.abs(l.y - sy) < 10)) continue;
                ...
                const textW = ctx.measureText(label).width + 6;
                ctx.fillStyle = "rgba(255,255,255,0.85)";
                ctx.fillRect(labelX2 - textW / 2, sy - fontSize / 2 - 2, textW, fontSize + 4);
                ctx.fillStyle = "rgba(113,113,122,0.8)";
                ctx.fillText(label, labelX2, sy);
                ctx.restore();
              }
            }
          }
